...
CREATE ALGORITHM=UNDEFINED DEFINER=`capstone2`@`%` SQL SECURITY DEFINER VIEW `POST_LIST` AS select `PB`.`BIG_ID` AS `BIG_ID`,`PB`.`PROJ_ID` AS `PROJ_ID`,`PB`.`BIG_LEVEL` AS `BIG_LEVEL`,`PB`.`BIG_TITLE` AS `BIG_TITLE`,`PB`.`BIG_START` AS `BIG_START`,`PB`.`BIG_END` AS `BIG_END`,`PB`.`BIG_DESC` AS `BIG_DESC`,`PB`.`BIG_ATTACHMENT` AS `BIG_ATTACHMENT`,`PB`.`BIG_STATUS` AS `BIG_STATUS`,`PB`.`BIG_AUTHOR` AS `BIG_AUTHOR`,`PB`.`BIG_CREATED` AS `BIG_CREATED`,`MIDJOIN`.`MID_ID` AS `MID_ID`,`MIDJOIN`.`MID_LEVEL` AS `MID_LEVEL`,`MIDJOIN`.`MID_TITLE` AS `MID_TITLE`,`MIDJOIN`.`MID_START` AS `MID_START`,`MIDJOIN`.`MID_END` AS `MID_END`,`MIDJOIN`.`MID_DESC` AS `MID_DESC`,`MIDJOIN`.`MID_ATTACHMENT` AS `MID_ATTACHMENT`,`MIDJOIN`.`MID_STATUS` AS `MID_STATUS`,`MIDJOIN`.`MID_AUTHOR` AS `MID_AUTHOR`,`MIDJOIN`.`MID_CREATED` AS `MID_CREATED`,`MIDJOIN`.`SML_ID` AS `SML_ID`,`MIDJOIN`.`SML_LEVEL` AS `SML_LEVEL`,`MIDJOIN`.`SML_TITLE` AS `SML_TITLE`,`MIDJOIN`.`SML_START` AS `SML_START`,`MIDJOIN`.`SML_END` AS `SML_END`,`MIDJOIN`.`SML_DESC` AS `SML_DESC`,`MIDJOIN`.`SML_ATTACHMENT` AS `SML_ATTACHMENT`,`MIDJOIN`.`SML_STATUS` AS `SML_STATUS`,`MIDJOIN`.`SML_AUTHOR` AS `SML_AUTHOR`,`MIDJOIN`.`SML_CREATED` AS `SML_CREATED` from (`capstoneDB`.`POST_BIG` `PB` join (select `PM`.`MID_ID` AS `MID_ID`,`PM`.`BIG_ID` AS `BIG_ID`,`PM`.`MID_LEVEL` AS `MID_LEVEL`,`PM`.`MID_TITLE` AS `MID_TITLE`,`PM`.`MID_START` AS `MID_START`,`PM`.`MID_END` AS `MID_END`,`PM`.`MID_DESC` AS `MID_DESC`,`PM`.`MID_ATTACHMENT` AS `MID_ATTACHMENT`,`PM`.`MID_STATUS` AS `MID_STATUS`,`PM`.`MID_AUTHOR` AS `MID_AUTHOR`,`PM`.`MID_CREATED` AS `MID_CREATED`,`PS`.`SML_ID` AS `SML_ID`,`PS`.`SML_LEVEL` AS `SML_LEVEL`,`PS`.`SML_TITLE` AS `SML_TITLE`,`PS`.`SML_START` AS `SML_START`,`PS`.`SML_END` AS `SML_END`,`PS`.`SML_DESC` AS `SML_DESC`,`PS`.`SML_ATTACHMENT` AS `SML_ATTACHMENT`,`PS`.`SML_STATUS` AS `SML_STATUS`,`PS`.`SML_AUTHOR` AS `SML_AUTHOR`,`PS`.`SML_CREATED` AS `SML_CREATED` from (`capstoneDB`.`POST_MID` `PM` join `capstoneDB`.`POST_SML` `PS` on((`PM`.`MID_ID` = `PS`.`MID_ID`)))) `MIDJOIN` on((`PB`.`BIG_ID` = `MIDJOIN`.`BIG_ID`)));
